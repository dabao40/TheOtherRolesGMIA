                ...
                if (!Lovers.existingWithKiller()) {
                    ...
                    foreach (PlayerControl p in CachedPlayer.AllPlayers) {
                        ...
                        if (p == Lovers.lover1 || p == Lovers.lover2)
                            ...
                            TempData.winners.Add(new WinningPlayerData(p.Data));
                        else if (p != Jester.jester && p != Jackal.jackal && p != Sidekick.sidekick && p != Arsonist.arsonist && p != Vulture.vulture && !Jackal.formerJackals.Contains(p) && !p.Data.Role.IsImpostor && p != Moriarty.moriarty && p != Akujo.akujo)
                            TempData.winners.Add(new WinningPlayerData(p.Data));
                    }
                }
                // Lovers solo win
