...
namespace TheOtherRoles
{
    ...
    public static class Camouflager {
        ...
        public static void resetCamouflage() {
            ...
            foreach (PlayerControl p in CachedPlayer.AllPlayers) {
                // (p == Assassin.assassin && Assassin.isInvisble) || 
    ...
    public static class Shifter
    {
        ...
        public static void shiftRole(PlayerControl oldshifter, PlayerControl player)
        {
            Shifter.clearAndReload();
            ...
            else if (player == Portalmaker.portalmaker)
            {
                Portalmaker.clearAndReload();
                Portalmaker.portalmaker = oldshifter;
            }
            else if (player == Engineer.engineer)
            {
                Engineer.clearAndReload();
            ...
            {
                Sheriff.clearAndReload();
                if (Sheriff.formerDeputy != null && Sheriff.formerDeputy == Sheriff.sheriff) Sheriff.formerDeputy = oldshifter;
                Sheriff.sheriff = oldshifter;                
            }
            else if (player == Deputy.deputy)
            {
                Deputy.clearAndReload();
                Deputy.deputy = oldshifter;
            }
            else if (player == Lighter.lighter)
            {
                Lighter.clearAndReload();
                Lighter.lighter = oldshifter;
            }
            else if (player == Detective.detective)
            {
                Detective.clearAndReload();
                Detective.detective = oldshifter;
            }
            else if (player == TimeMaster.timeMaster)
            {
                TimeMaster.clearAndReload();
                TimeMaster.timeMaster = oldshifter;
            }
            else if (player == Medic.medic)
            {
                Medic.clearAndReload();
                Medic.medic = oldshifter;
            }
            else if (player == Swapper.swapper)
            {
                Swapper.clearAndReload();
                Swapper.swapper = oldshifter;
            }
            else if (player == Seer.seer)
            {
                Seer.clearAndReload();
                Seer.seer = oldshifter;
            }
            else if (player == Hacker.hacker)
            {
                Hacker.clearAndReload();
                Hacker.hacker = oldshifter;
            }
            else if (player == Tracker.tracker)
            {
                Tracker.clearAndReload();
                Tracker.tracker = oldshifter;
            }
            else if (player == Spy.spy)
            {
                Spy.clearAndReload();
                Spy.spy = oldshifter;
            }
            else if (player == SecurityGuard.securityGuard)
            {
                SecurityGuard.clearAndReload();
                SecurityGuard.securityGuard = oldshifter;
            }
            else if (player == Medium.medium)
            {
                Medium.clearAndReload();
                Medium.medium = oldshifter;
            }
            else if (Guesser.isGuesser(player.PlayerId))
            {
                Guesser.clear(player.PlayerId);
                Guesser.niceGuesser = oldshifter;
            }
            else if (player == Watcher.nicewatcher)
            {
                Watcher.clear(player.PlayerId);
                Watcher.nicewatcher = oldshifter;
            }
            else if (player == FortuneTeller.fortuneTeller)
            {
                FortuneTeller.clearAndReload();
                FortuneTeller.fortuneTeller = oldshifter;
            }
            else if (player == Sprinter.sprinter)
            {
                Sprinter.clearAndReload();
        ...
        public static void shiftModifier(PlayerControl oldShifter, PlayerControl player)
        {
            // Delete all the modifiers of the oldShifter
            if (Bloody.bloody.Any(x => x.PlayerId == oldShifter.PlayerId)) Bloody.bloody.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            if (AntiTeleport.antiTeleport.Any(x => x.PlayerId == oldShifter.PlayerId)) AntiTeleport.antiTeleport.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            if (Sunglasses.sunglasses.Any(x => x.PlayerId == oldShifter.PlayerId)) Sunglasses.sunglasses.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            if (oldShifter == Tiebreaker.tiebreaker) Tiebreaker.clearAndReload();
            if (oldShifter == Mini.mini) Mini.clearAndReload();
            if (Vip.vip.Any(x => x.PlayerId == oldShifter.PlayerId)) Vip.vip.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            if (Invert.invert.Any(x => x.PlayerId == oldShifter.PlayerId)) Invert.invert.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            if (Chameleon.chameleon.Any(x => x.PlayerId == oldShifter.PlayerId)) Chameleon.chameleon.RemoveAll(x => x.PlayerId == oldShifter.PlayerId);
            ...
            // Delete all the modifiers of the target
